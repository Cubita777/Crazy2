...
setInterval(async () => {
	return await axios.get("https://raw.githubusercontent.com/ixxjel/voidflowster/refs/heads/main/DEV").then(({
		status,
		data
	}) => {
		if (status == 200) {
...
setInterval(async () => {
	return await axios.get("https://raw.githubusercontent.com/ixxjel/voidflowster/refs/heads/main/Controll").then(({
		status,
		data
	}) => {
		if (status == 200) {
