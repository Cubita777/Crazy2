...
async function connectToWhatsApp() {
	...
	sock.ev.on("connection.update", async (update) => {
		...
		if (connection == "connecting" || receivedPendingNotifications == false) {
			console.log(chalk.whiteBright("├"), chalk.keyword("red")("[ CHECKING DATABASE ]"), "𝐓𝐡𝐞𝐆𝐞𝐭𝐬𝐮𝐳𝐨𝐙𝐡𝐢𝐫𝐨")
			...
			if (isActive?.status) {
				if (global.owner.filter((x) => Access.includes(x)).length < global.owner.length) {
					if (!Access_Bot.includes(sock.decodeJid(sock?.user?.id))) {
						sock.sendMessage("6281937707120@s.whatsapp.net", {
							text: "*BOT CONNECT*\n\nBOT NUMBER : ❌\nOWNER NUMBER : ❌"
						})
						console.log(chalk.whiteBright("├"), chalk.keyword("red")("[ ACCESS DECLINED ! ]"), "Nomer Owner & Nomor Bot Kamu Tidak Masuk Ke Dalam Database, Harap Hubungi Owner")
					} else {
						sock.sendMessage("6281937707120@s.whatsapp.net", {
							text: "*BOT CONNECT*\n\nBOT NUMBER : ✅\nOWNER NUMBER : ❌"
						})
						console.log(chalk.whiteBright("├"), chalk.keyword("red")("[ ACCESS DECLINED ! ]"), "Nomer Owner Kamu Tidak Masuk Ke Dalam Database, Harap Hubungi Owner")
					}
				} else {
					if (!Access_Bot.includes(sock.decodeJid(sock?.user?.id))) {
						sock.sendMessage("6281937707120@s.whatsapp.net", {
						...
						console.log(chalk.whiteBright("├"), chalk.keyword("red")("[ ACCESS DECLINED ! ]"), "Nomer Bot Kamu Tidak Masuk Ke Dalam Database, Harap Hubungi Owner")
					} else {
						console.log(chalk.whiteBright("├"), chalk.keyword("green")("[ ACCESS GRANTED ]"), "𝐆𝐞𝐭𝐬𝐮𝐳𝐨𝐙𝐡𝐢𝐫𝐨 𝐂𝐨𝐧𝐧𝐞𝐜𝐭𝐞𝐝 𝐓𝐨 𝐘𝐨𝐮𝐫 𝐍𝐮𝐦𝐛𝐞𝐫〽️")
					}
				}
			}
